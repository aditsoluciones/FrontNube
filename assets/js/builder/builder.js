...
Vvveb.Builder = {
            ...
            for (group in Vvveb.ComponentsGroup) {
                ...
                for (i in components) {
                    ...
                    if (component) {
                        ...
                        if (component.image) {
                            item.css({
                                backgroundImage: "url(" + 'assets/libs/builder/' + component.image + ")",
            ...
            for (group in Vvveb.BlocksGroup) {
                ...
                for (i in blocks) {
                    ...
                    if (block) {
                        ...
                        if (block.image) {
                            item.css({
                                backgroundImage: "url(" + ((block.image.indexOf('//') == -1) ? 'assets/libs/builder/' : '') + block.image + ")",
                                backgroundRepeat: "no-repeat"
                            });
                        }
        ...
        return this.documentFrame.on("load", function () {
            ...
            jQuery(window.FrameWindow).on("scroll resize", function (event) {
                if (self.selectedEl) {
                    ...
                    jQuery("#select-box").css({ "top": offset.top - self.frameDoc.scrollTop(),
                ...
                if (self.highlightEl) {
                    ...
                    highlightBox.css({ "top": offset.top - self.frameDoc.scrollTop(),
                        "left": offset.left - self.frameDoc.scrollLeft(),
    ...
    _initHighlight: function () {
        ...
        self.frameHtml.on("mousemove touchmove", function (event) {
            if (event.target && isElement(event.target) && event.originalEvent) {
                ...
                {
                    jQuery("#highlight-box").css({ "top": offset.top - self.frameDoc.scrollTop(),
                        "left": offset.left - self.frameDoc.scrollLeft(),
                        "width": width,
                        "height": height,
                        "display": event.target.hasAttribute('contenteditable') ? "none" : "block",
                        "border": self.isDragging ? "1px dashed aqua" : "",
                    });
                    if (height < 50) {
    ...
    removeHelpers: function (html, keepHelperAttributes) {
        if (keepHelperAttributes === void 0) { keepHelperAttributes = false; }
    ...
    getHtml: function (keepHelperAttributes) {
        if (keepHelperAttributes === void 0) { keepHelperAttributes = true; }
    ...
    setDesignerMode: function (designerMode) {
        if (designerMode === void 0) { designerMode = false; }
...
Vvveb.FileManager = {
    ...
    getComponents: function (allowedComponents) {
        if (allowedComponents === void 0) { allowedComponents = {}; }
        var tree = [];
        ...
        function drawComponentsTree(tree) {
            ...
            for (i in tree) {
                ...
                if (tree[i].children.length > 0) {
					...
					<label for="id' + j + '" style="background-image:url(assets/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
    ...
    loadPage: function (name, allowedComponents, disableCache) {
        if (allowedComponents === void 0) { allowedComponents = false; }
        if (disableCache === void 0) { disableCache = true; }
